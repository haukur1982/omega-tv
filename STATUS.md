...
- **Orð dagsins (daily word) built** (`e9d07d2`, NOT yet deployed): `daily_words` table + `getDailyWord` + a card beside Bæn dagsins in the daily block (two fresh things each morning, the move that makes the front door a daily habit) + admin scheduler `/admin/ord-dagsins` (load weeks ahead = a buffer, never a daily deadline). 3 starter examples flagged for Hawk's review; verse-text fields left empty for the licensed Bible / translated PD devotional content (e.g. Spurgeon "Kvöld og morgunn"). Goes live on the next deploy.
